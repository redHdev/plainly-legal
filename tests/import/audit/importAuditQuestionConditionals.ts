...
export async function importClauses(): Promise<void> {
  const csvFolderPath = path.join(__dirname + '/../../../../../tests/import/audit/auditQuestionConditionals');
  ...
  for (const file of files) {
    ...
    for (const data of readyToImportData) {
      ...
      const existingData = await prisma.auditQuestionConditionals.findUnique({
        where: { key: data.key },
      });
      ...
      await prisma.auditQuestionConditionals.create({ data: newData });
