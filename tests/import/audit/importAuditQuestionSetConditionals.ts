...
export async function importClauses(): Promise<void> {
  const csvFolderPath = path.join(__dirname + '/../../../../../tests/import/audit/auditQuestionSetConditionals');
  ...
  for (const file of files) {
    ...
    const dataArray = parsedCsv.map(data => {
      return {
        ...
        question: data['Question ID']?.trim(),
    ...
    const readyToImportData = dataArray.filter(data => {
      if (data.key && data.operand && data.termOne && data.termTwo && data.question) {
        return true;
      } else {
    ...
    for (const data of readyToImportData) {
      ...
      const existingData = await prisma.auditQuestionSetConditionals.findUnique({
        where: { key: data.key },
      });
      ...
      if (data.question) {
        questions = data.question.split(',').map(record => {
      ...
      await prisma.auditQuestionSetConditionals.create({ data: newData });
