...
export async function importClauses(): Promise<void> {
  const csvFolderPath = path.join(__dirname + '/../../../../../tests/import/audit/auditQuestionSets/');
  ...
  for (const file of files) {
    ...
    const readyToImportData = dataArray.filter(data => {
      if (data.key && data.setCode && data.answerType) {
        return true;
      } else {
    ...
    for (const data of readyToImportData) {
      ...
      const existingData = await prisma.auditQuestionSets.findUnique({
        where: { key: data.key },
      });
      ...
      await prisma.auditQuestionSets.create({ data: { ...data } });
